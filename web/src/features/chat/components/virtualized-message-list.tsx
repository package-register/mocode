...
export type VirtualizedMessageListProps = {
  ...
  /** Toggle selection of a message by ID */
  ...
  return (
    <Virtuoso
      ...
      itemContent={(_index, item) => {
        ...
        const isSelectable = true; // status/message-id already filtered out above
        const isSelected = isSelectMode && selectedIds?.has(message.id);

        return (
          <div className={cn("relative group", isSelected && "bg-primary/5 rounded-lg")}>
          {isSelectMode && isSelectable && (
            <button
              type="button"
              className="absolute left-0 top-3 z-10 text-muted-foreground hover:text-foreground transition-colors"
          ...
          <Message
            className={cn(
              ...
              isSelectMode && isSelectable && "pl-6",
            )}
            from={message.role}
          >
            {message.role === "user" ? (

...
func TestMocodeignore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .mocodeignore file that ignores .log files
	require.NoError(t, os.WriteFile(".mocodeignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	mocodeignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".mocodeignore"), []byte(mocodeignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .mocodeignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .mocodeignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .mocodeignore in subdir that ignores normal_nested
	submocodeignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".mocodeignore"), []byte(submocodeignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .mocodeignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .mocodeignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .mocodeignore")

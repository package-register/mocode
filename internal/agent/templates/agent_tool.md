...
1a. You can also pass `tasks` to this tool to run multiple sub-agent tasks in one parallel batch. Each task supports `prompt`, optional `agent_name`, and optional `allow_edit`.
...
6. Set `allow_edit: true` only for a sub-agent that should be allowed to use edit/write/multiedit; leave it false for read-only research.
</usage_notes>

...
// TODO: reimplement using file-based store (see .skcode/plans/sqlite-to-file-storage/)
...
func hyperBuilder(model string) builderFunc {
	return func(t *testing.T, r *vcr.Recorder) (fantasy.LanguageModel, error) {
		provider, err := openaicompat.New(
			...
			openaicompat.WithAPIKey(os.Getenv("MOCODE_HYPER_API_KEY")),
...
func testEnv(t *testing.T) fakeEnv {
	workingDir := filepath.Join("/tmp/Mocode-test/", t.Name())
...
func coderAgent(r *vcr.Recorder, env fakeEnv, large, small fantasy.LanguageModel) (SessionAgent, error) {
	...
	// independently of user config on `$HOME/.config/mocode/mocode.json`.
	...
	cfg.Config().Options.DisabledSkills = []string{"mocode-config"}

...
import (
	...
	_ "github.com/package-register/mocode/internal/swagger"
...
func DefaultHost() string {
	...
	if err == nil && usr.Uid != "" {
		sock = fmt.Sprintf("mocode-%s.sock", usr.Uid)
	}
...
// Server represents a mocode server bound to a specific address.
type Server struct {
...
func NewServer(cfg *config.ConfigStore, network, address string) *Server {
	...
	mux.HandleFunc("PUT /v1/workspaces/{id}/messages/{mid}", c.handlePutWorkspaceMessage)
	mux.HandleFunc("DELETE /v1/workspaces/{id}/messages/{mid}", c.handleDeleteWorkspaceMessage)

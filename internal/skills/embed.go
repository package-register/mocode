...
const BuiltinPrefix = "mocode://skills/"
...
func DiscoverBuiltinWithStates() ([]*Skill, []*SkillState) {
	...
	fs.WalkDir(builtinFS, "builtin", func(path string, d fs.DirEntry, err error) error {
		...
		// Set paths using the mocode prefix. Strip the leading "builtin/"
		// to make paths relative to the embedded root
		// (e.g., "mocode://skills/mocode-config/SKILL.md").

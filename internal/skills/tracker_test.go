...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (mocode-config).
	activeSkills := []*Skill{
		{Name: "mocode-config", Description: "Mocode config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("mocode-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via mocode://...).
	tracker.MarkLoaded("mocode-config")
	require.True(t, tracker.IsLoaded("mocode-config"))

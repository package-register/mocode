...
func BuildEnv(eventName, toolName, sessionID, cwd, projectDir, toolInputJSON string) []string {
	env := os.Environ()
	env = append(env,
		fmt.Sprintf("MOCODE_EVENT=%s", eventName),
		fmt.Sprintf("MOCODE_TOOL_NAME=%s", toolName),
		fmt.Sprintf("MOCODE_SESSION_ID=%s", sessionID),
		fmt.Sprintf("MOCODE_CWD=%s", cwd),
		fmt.Sprintf("MOCODE_PROJECT_DIR=%s", projectDir),
	)

	// Extract tool-specific env vars from the JSON input.
	if toolInputJSON != "" {
		if cmd := gjson.Get(toolInputJSON, "command"); cmd.Exists() {
			env = append(env, fmt.Sprintf("MOCODE_TOOL_INPUT_COMMAND=%s", cmd.String()))
		}
		if fp := gjson.Get(toolInputJSON, "file_path"); fp.Exists() {
			env = append(env, fmt.Sprintf("MOCODE_TOOL_INPUT_FILE_PATH=%s", fp.String()))
		}
	}
...
// Supports both Mocode format and Claude Code format (hookSpecificOutput).
...
func rawToString(raw json.RawMessage) string {
	...
	// Otherwise it's an object/array 鈥?use as-is.
	return string(raw)
}
